function MultiselectDropdown(options){
  ...
  document.querySelectorAll("select[multiple]").forEach((el,k)=>{
    ...
    var div=newEl('div',{class:'multiselect-dropdown-custom',style:{width:config.style?.width??el.clientWidth+'px',padding:config.style?.padding??''}});
    ...
    var search=newEl('input',{class:['multiselect-dropdown-custom-search'].concat([config.searchInput?.class??'form-control']),style:{width:'100%',display:el.attributes['multiselect-search']?.value==='true'?'block':'none'},placeholder:config.txtSearch});
